...
fun FirPropertyBuilder.generateAccessorsByDelegate(
     ...
     *   generator function
     */
    fun thisRef(dispatchReceiverFirst: Boolean = false): FirExpression =
        when {
            isExtension && !dispatchReceiverFirst -> buildThisReceiverExpression {
                source = fakeSource
    ...
    fun delegateAccess() = buildQualifiedAccessExpression {
        ...
        if (ownerSymbol != null) {
            dispatchReceiver = thisRef(dispatchReceiverFirst = true)
        }
    }
    ...
    delegateBuilder.delegateProvider = if (stubMode) buildExpressionStub() else buildFunctionCall {
        ...
        argumentList = buildBinaryArgumentList(thisRef(dispatchReceiverFirst = true), propertyRef())
    }
